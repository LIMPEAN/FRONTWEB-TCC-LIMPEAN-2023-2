...
export function CardDiarista({urlImagem, nome, biografia, valor}: CardDiaristaProps){
    return(
        <div className="flex flex-col items-center gap-4 p-4 h-fit w-64 bg-white drop-shadow-mdrounded-md">
            <Image className="w-32 h-32 rounded-full" src={urlImagem} height={100} width={100} alt={nome}/>
            <h1>{nome}</h1>
            <span className="flex text-xs text-zinc-400 text-center h-16 overflow-y-auto">{biografia}</span>
            <div className="flex gap-2 text-zinc-600">
                <span className="font-medium">Média:</span>
            <span className="font-light">R$ {valor}</span>
            </div>
