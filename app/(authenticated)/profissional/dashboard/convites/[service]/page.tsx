...
import { ClientData, ResponseService } from "./interfaces/baseResponseService";
import StreetViewImage from "./components/streetViewImage";
import { PutService } from "./interfaces/putService";
import { putPrice } from "./services/putPrice";
...
export default function ServiceModal({
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      fetch(apiUrl, { headers })
        ...
        .then((data: ResponseService) => {
          data.data.map((client: ClientData) => {
            if (client.client.serviceId === Number(params.service)) {
  ...
  const fetchPutApi = async () => {
    ...
    const jsonPutService: PutService = {
      idService: Number(params.service),
      newValue: price ? price.toString() : '0.00'
    }
    ...
    if (response.status === 201) {
      ...
      toast.error("Falha na definição do preço, verifique se o valor é superior a R$1000!")
    }
  }
