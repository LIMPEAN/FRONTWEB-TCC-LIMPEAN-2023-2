...
import { IData, IDataClient } from "../interfaces/baseResponseService";
...
export default function ServiceModal({
  ...
  const [responseData, setResponseData] = useState<IDataClient | null>(null);
    ...
    const fetchData = () => {
      ...
      fetch(apiUrl, { headers })
        ...
        .then((data) => {
          ...
          data.data.map((data: IDataClient) => {
            if (data.client.serviceId === Number(params.detalhes)) {
              console.log(data)
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col lg:pb-4 pb-14">
      ...
      <div className="flex flex-col lg:flex-row lg:overflow-y-hidden h-full items-center p-4 text-gray-800 rounded-lg shadow dark:bg-gray-800 dark:text-white dark:border-gray-700 gap-8">
        {/* {responseData ? <StreetViewImage client={responseData.client}
        /> : <LoadingImage />} */}
        <Image className="lg:h-fit lg:w-1/2 w-full" src="/assets/agendados/agendado_detalhe.svg" alt="vetor" width={300} height={300} />
        <div className="flex flex-col gap-2  justify-start  overflow-y-scroll lg:h-full lg:w-1/2 w-full">
          <h3 className="text-3xl 2xl:text-5xl font-extrabold mb-4">Dados da faxina</h3>
          <div className="flex flex-col">
            <span className="font-semibold text-base 2xl:text-lg">Tipo de faxina</span>
            <div className="flex gap-1 items-center">
              <input id="radio" type="radio" checked />
              <label className="2xl:text-lg" htmlFor="radio">{responseData?.client.type_clean}</label>
            </div>
          </div>
          <div className="flex flex-col">
            <span className="font-semibold text-base 2xl:text-lg">{responseData?.client.question[0].asks}</span>
            <div className="flex gap-1 items-center">
              <input id="radio" type="radio" checked />
              <label className="2xl:text-lg" htmlFor="radio">{responseData?.client.question[0].answer ? "Sim" : "Não"}</label>
            </div>
          </div>
          <div className="flex flex-col font-semibold">
            ...
            <div className="flex mb-2 gap-2">
              {responseData?.client.room[0].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[0].quantity!!}</span>
              </div> : null}
              {responseData?.client.room[1].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[1].quantity}</span>
              </div> : null}
              {responseData?.client.room[2].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[2].quantity}</span>
              </div> : null}
              {responseData?.client.room[3].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[3].quantity!!}</span>
              </div> : null}
              {responseData?.client.room[4].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[4].quantity}</span>
              </div> : null}
              {responseData?.client.room[5].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[5].quantity}</span>
              </div> : null}
              {responseData?.client.room[6].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[6].quantity}</span>
              </div> : null}
              {responseData?.client.room[7].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[7].quantity}</span>
              </div> : null}
              {responseData?.client.room[8].quantity!! > 0 ? <div className="flex gap-1 items-center">
                ...
                <span>{responseData?.client.room[8].quantity}</span>
              </div> : null}
            </div>
          </div>
          <div className="flex flex-col">
            <span className="font-semibold text-base 2xl:text-lg">{responseData?.client.question[1].asks}</span>
            <div className="flex gap-1 items-center">
              <input id="radio" type="radio" checked />
              <label className="2xl:text-lg" htmlFor="radio">{responseData?.client.question[1].answer ? "Sim" : "Não"}</label>
            </div>
          </div>
            ...
            <div className="flex gap-2">
              <div className="w-full">
                ...
                <Datepicker id="data" className="w-full" disabled value={
                  responseData ? `${new Date(responseData.client.date_hour).getDay()}/${new Date(responseData.client.date_hour).getMonth()}/${new Date(responseData.client.date_hour).getFullYear()}` : "null"} />
              </div>
              <div className="w-full">
                ...
                <TextInput id="time" className="w-full"
                  disabled type="text" icon={IconClock} value={
                    responseData ? `${new Date(responseData.client.date_hour).getHours()}:${new Date(responseData.client.date_hour).getMinutes()}` : "null"} />
              </div>
            </div>
            <div className="w-full">
              ...
              {/* <iframe
                ...
                loading="lazy"
                allowFullScreen
                src={`https://www.google.com/maps/embed/v1/directions?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&origin=06655-450&destination=${responseData?.client.address.cep}`}
              ></iframe> */}
              <div>
                {responseData ? <StreetViewImage service={responseData.client}
                /> : <LoadingImage />}
              </div>
            </div>
            <div className="w-full">
              ...
              <TextInput id="data" className="w-full"
                ...
                addon="R$"
                disabled
                value={responseData?.client?.value + ".00" ?? "0.00"}
              />
            </div>
            <div className="w-full flex gap-2">
              <Link
                href="../"
                className='h-12 w-full bg-transparent border border-red-500 text-red-500 hover:bg-red-500 hover:text-white flex text-center justify-center items-center custom-file-label font-medium rounded-lg'
              >
                Cancelar
              </Link>
              <Link 
